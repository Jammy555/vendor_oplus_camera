...
patch_camera_apk() {
    ...
    if [ ! -d "$decompiled_dir" ]; then
        ...
        java -jar "$apktool_jar" d "${apk_dir}/OplusCamera.apk" -o "$decompiled_dir"
        rm -f "$marker"
    fi
    ...
    if [ ! -f "$marker" ] || [ $patch_status -eq 10 ]; then
        ...
        java -jar "$apktool_jar" b "$decompiled_dir" -o "${apk_dir}/OplusCamera.apk"
        touch "$marker"
...
patch_camera_sdk() {
    ...
    if [ ! -d "$decompiled_dir" ]; then
        ...
        java -jar "$apktool_jar" d "${sdk_dir}/com.oplus.camera.unit.sdk.jar" -o "$decompiled_dir"
        rm -f "$marker"
    fi
    ...
    if [ ! -f "$marker" ] || [ $patch_status -eq 10 ]; then
        ...
        java -jar "$apktool_jar" b "$decompiled_dir" -o "${sdk_dir}/com.oplus.camera.unit.sdk.jar"
        touch "$marker"
